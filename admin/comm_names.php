...
if(isset($_POST['add_commitee_name']))
{

$title = mysqli_real_escape_string($db, $_POST['commitee_title']);

// Check if committee with same title already exists
$chkquery = "SELECT * FROM commitee WHERE title = '$title'";
$chkresult = mysqli_query($db, $chkquery);

if(mysqli_num_rows($chkresult) == 0) {
    // Prepare and execute SQL query (using prepared statement)
    $sql = "INSERT INTO commitee (title) VALUES (?)";
    ...
    if(mysqli_stmt_affected_rows($stmt) > 0) {
        ...
        $_SESSION['error'] = "Failed to add new committee!";
    }

    // Close prepared statement
    mysqli_stmt_close($stmt);
} else {
    // Display error message if committee already exists
    $_SESSION['error'] = "Committee with same title already exists!";
}
...
<!--                </div>-->
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0 text-dark">Commitee Names</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="home.php">Home</a></li>
                                <li class="breadcrumb-item active">Comm. Names</li>
                            </ol>
                        </div>
                    </div>
                </div>
...
//      ?>
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <div align="right">
                                    <button class="btn btn-primary btn-flat" data-toggle="modal" data-target="#modal-default"><i class="fas fa-plus"></i> Add New</button>
                                </div><br>
                ...
                ?>
                                        <tr>
                                            ...
                                            <td>
                                                <button class="btn btn-success btn-flat com_edit" id="<?php echo $row['com_id']; ?>"><i class="fas fa-edit"></i></button>
                                                <button class="btn btn-danger btn-flat com_delete" id="<?php echo $row['com_id']; ?>"><i class="fas fa-trash"></i></button>
                                            </td>
                                        </tr>
                                        <?php
                }
                ...
                <div class="modal-body">
                    <form method="POST">
                        ...
                        <div class="form-group row">
                            ...
                            <label class="col-sm-3 col-form-label">Committee Name</label>
                            <div class="col-sm-7">
                                <input type="text" class="form-control" name="commitee_title" placeholder="" required>
                            </div>
                        </div>
...
<!--                        </div>-->
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <button type="submit" class="btn btn-primary btn-flat" name="add_commitee_name"><i class="fas fa-save"></i> Save</button>
                    </form>
                </div>
            </div>
            <!-- /.modal-content -->
    ...
    <div id="com_edit_modal" class="modal fade">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Update Commitee Name </h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form method="POST">
                    <div class="modal-body" id="com_edit_details">
                    </div>
                    <div class="modal-body"></div>
                    <div class="modal-footer justify-content-between">
                        <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i class="fas fa-times"></i> Close</button>
                        <button type="submit" class="btn btn-primary btn-flat" name="com_update"><i class="fas fa-check"></i> Update</button>
                </form>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
        $('.com_edit').click(function() {
            var com_update_id = $(this).attr("id");
            $.ajax({
                url: "controller.php",
                method: "post",
                data: {
                    com_update_id: com_update_id
                },
                success: function(data) {
                    $('#com_edit_details').html(data);
                    $('#com_edit_modal').modal("show");
                }
            });
        });
    });
...
<!--==================================DELETE COMMITEE========================================================-->
    <div id="com_delete_modal" class="modal fade">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form method="POST">
                    <div class="modal-body" id="com_delete_details">
                    </div>
                    <div class="modal-body"></div>
                    <div class="modal-footer justify-content-between">
                        <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i class="fas fa-times"></i> Close</button>
                        <button type="submit" class="btn btn-danger btn-flat" name="com_delete"><i class="fas fa-trash"></i> Delete</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    $(document).ready(function() {
        $('.com_delete').click(function() {
            var com_del_id = $(this).attr("id");
            $.ajax({
                url: "controller.php",
                method: "post",
                data: {
                    com_del_id: com_del_id
                },
                success: function(data) {
                    $('#com_delete_details').html(data);
                    $('#com_delete_modal').modal("show");
                }
            });
        });
    });
