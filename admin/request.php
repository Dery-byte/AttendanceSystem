...
<!--                            </div><br>-->
                            <table id="example1" class="table table-bordered dataTable no-footer" role="grid" aria-describedby="example1_info">
                                ...
                                <tbody>
                                <?php
                                $sql = "SELECT * FROM senate_attendance, senate_list, senate_sched WHERE senate_attendance.senator_id = senate_list.student_id AND senate_attendance.schedule = senate_sched.sched_id AND approve_statuses=0";
                                ...
                                while($row = mysqli_fetch_array($result))
                                {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button class="btn btn-danger btn-flat reject_request" id="<?php echo $row['attendance_id']; ?>">Reject</button>
                                        </td>
                                    </tr>
                                    <?php
                                }
...
<div id="pos_confirm_modal" class="modal fade">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Confirm Choice... </h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <form method="POST">
                <div class="modal-body" id="pos_confirm_details">
                </div>
                <div class="modal-body"></div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i class="fas fa-times"></i> Close</button>
                    <button type="submit" class="btn btn-primary btn-flat" name="pos_confirm"><i class="fas fa-check"></i> Approve</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('.pos_confirm').click(function() {
            var attendanace_id = $(this).attr("id");
            $.ajax({
                url: "controller.php",
                method: "post",
                data: {
                    attendanace_id: attendanace_id
                },
                success: function(data) {
                    $('#pos_confirm_details').html(data);
                    $('#pos_confirm_modal').modal("show");
                }
            });
        });
    });
...
<div id="reject_modal" class="modal fade">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body" id="reject_details">
                </div>
                <div class="modal-body"></div>
                <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-default btn-flat" data-dismiss="modal"><i class="fas fa-times"></i> Close</button>
                    <button type="submit" class="btn btn-danger btn-flat" name="reject_request"><i class="fas fa-trash"></i> Reject</button>
                </div>
            </form>
        </div>
    </div>
